...
RUN DEBIAN_FRONTEND=noninteractive && \
	apt-get update && \
	apt-get install -y tzdata busybox openssl libssl-dev devscripts gcc-8 g++-8 && \
	ls -1 /usr/bin/{gcc,cpp,g++}*-8 | while read F; do ln -sf $F $(sed 's/-8$//g' <<< $F); done && \
	ls -1 /usr/bin/x86_64-linux-gnu-*-8 | while read F; do ln -sf $F $(sed 's/-8$//g' <<< $F); done && \
	...
	apt-get install -y -f && \
	echo "---------------------------------------------------------------------------------"&& \
	apt-get remove --purge --auto-remove -y squid-build-deps devscripts gcc-8 g++-8 && \
	apt-get clean && apt-get autoclean && \
	rm -rf /var/lib/apt/lists/* && \
